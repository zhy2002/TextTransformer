...
namespace ZCL.RTScript.Logic.Expression.Literal
{
    ...
    /// </summary>
    internal class ReplaceExpression : IReplaceContext
    {
        public int MatchId
        {
            get;
        ...
        private object Execute(string expr, ref int index)
        {
            ...
            while (true) //get all parms
            {
                ...
                if (tokenStartIndex >= expr.Length) throw new ReplaceException("Unexpected end of expression") { SourceCode = expr, SourceIndex = tokenStartIndex };
                TokenType token = ReadToken(expr, ref index);
                ...
                else if (state == ParsingState.FuncName)
                {
                    ...
                    if (token != TokenType.Literal) throw new ReplaceException("Expecting a function name at the beginning of an expression") { SourceCode = expr, SourceIndex = tokenStartIndex };
                    try
                    {
                ...
                else if (state == ParsingState.ParmEnd)
                {
                    if (token == TokenType.ExprEnd) break;
                    if (token == TokenType.WhiteSpace) continue;
                    if (token != TokenType.Separator) throw new ReplaceException("Expecting a separator") { SourceCode = expr, SourceIndex = tokenStartIndex };
                    state = ParsingState.ParmStart;
                }
            }
            ...
            {
                throw new ReplaceException(ex.Message, ex) { SourceCode = expr, SourceIndex = exprStartIndex };
                throw ex;
            }
        }
        ...
        private object Execute(List<object> parms)
        {
            if (parms[0] is double)
            {
                ...
                if (index >= MatchValues.Count) throw new ReplaceException("Index '" + index + "' is out of range for matched value accessor");
                return MatchValues.GetItem(index);
            }
            ...
            if (funcName != null && funcName[0] != '"')
            {
                ...
                {
                    if (parms.Count - 1 < func.MinArgNumber) throw new ReplaceException("Function {OpName} needs at least {MinArgNumber} arguments but got {ActualArgNumber} in expression") { OpName = funcName, MinArgNumber = func.MinArgNumber, ActualArgNumber = parms.Count - 1 };
                    if (parms.Count - 1 > func.MaxArgNumber) throw new ReplaceException("Function {OpName} can have at most {MaxArgNumber} arguments but got {ActualArgNumber} in expression") { OpName = funcName, MaxArgNumber = func.MaxArgNumber, ActualArgNumber = parms.Count - 1 };
                    return func.Body(parms, this);
                }
            }

            throw new ReplaceException("Function name {OpName} is not found in expression") { OpName = funcName };
        }
        ...
        private void ParseLiteral(string rawValue, out object literal)
        {
            ...
            if (rawValue[0] == '"') //check string
            {
                ...
                if (endQuotes % 2 != 1) throw new ReplaceException("String <" + rawValue + "> has missing end quote.");
